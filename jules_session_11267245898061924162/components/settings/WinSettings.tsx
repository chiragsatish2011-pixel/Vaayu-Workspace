...
export function WinSettings({ user }: { user: WinUser }) {
  ...
  return (
    <div className="flex min-h-[calc(100dvh-118px)] flex-col bg-canvas md:flex-row">
      {/* Left - Windows sidebar */}
      <aside className="w-full shrink-0 border-b border-hairline-soft bg-fog p-3 md:w-[280px] md:border-b-0 md:border-r">
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition-colors ${
                    isActive ? "bg-canvas shadow-xs border border-hairline-soft text-ink" : "text-charcoal hover:bg-mist hover:text-ink"
                  }`}
                >
                  <span className={`grid h-7 w-7 place-items-center rounded-md ${isActive ? "bg-azure text-white" : "bg-canvas border border-hairline-soft text-steel"} `}>
        ...
        {/* Right - main */}
        <div className="min-w-0 flex-1 bg-fog/40 p-4 sm:p-8">
...
function HomePanel({ user, onNav }: { user: WinUser; onNav: (id: NavId) => void }) {
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-4 rounded-xl border border-hairline bg-canvas p-4 shadow-sm">
        <div className="h-14 w-20 overflow-hidden rounded-lg border border-hairline bg-ink grid place-items-center">
          <span className="font-display text-lg font-bold tracking-tight text-canvas">{getDisplayName(user.displayName, user.email).slice(0, 2).toUpperCase()}</span>
        </div>
        <div className="min-w-0 flex-1">
          ...
          <button onClick={() => onNav("accounts")} className="text-xs font-semibold text-azure hover:underline">
            Rename / Edit profile
          </button>
        </div>
        <div className="hidden shrink-0 items-center gap-4 sm:flex">
          <div className="flex items-center gap-2 text-xs">
            <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-azure-soft text-azure">◈</span>
            <span className="leading-tight">
              <span className="block font-semibold text-ink">Drive</span>
          ...
          <div className="flex items-center gap-2 text-xs">
            <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-azure-soft text-azure">●</span>
            <span className="leading-tight">
              <span className="block font-semibold text-ink">Vercel + Neon</span>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.7fr_0.9fr]">
        ...
        <div className="rounded-xl border border-hairline bg-canvas p-5 shadow-sm">
          <div className="flex items-center gap-2">
            <span className="grid h-6 w-6 place-items-center rounded bg-fog text-xs">▦</span>
            <p className="text-sm font-semibold text-ink">Your Vaayu account</p>
          </div>
          <p className="mt-1 text-lg font-bold text-ink">It’s all here with your profile</p>
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-ink">{getDisplayName(user.displayName, user.email)}</p>
              <p className="font-mono text-xs text-steel">{user.jobTitle || "Set your title"}</p>
            </div>
            <button onClick={() => onNav("accounts")} className="ml-auto rounded-full border border-hairline bg-canvas px-4 py-1.5 text-xs font-semibold text-ink hover:border-ink">
              Manage
            </button>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-hairline bg-canvas shadow-sm">
          <div className="border-b border-hairline-soft p-4">
            <p className="text-sm font-semibold text-ink">Recommended settings</p>
          ...
          <button onClick={() => onNav("system")} className="flex w-full items-center gap-3 border-b border-hairline-soft px-4 py-3 text-left hover:bg-fog/50">
            ...
            <span className="text-sm text-ink">System — Uploads</span>
          ...
          <button onClick={() => onNav("accounts")} className="flex w-full items-center gap-3 border-b border-hairline-soft px-4 py-3 text-left hover:bg-fog/50">
            ...
            <span className="text-sm text-ink">Accounts — Profile</span>
            <span className="ml-auto text-stone">›</span>
          </button>
          <button onClick={() => onNav("security")} className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-fog/50">
            ...
            <span className="text-sm text-ink">Privacy & security — Password</span>
            <span className="ml-auto text-stone">›</span>
          </button>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-hairline bg-canvas p-4 shadow-sm">
        <p className="text-sm font-semibold text-ink">Personalize your workspace</p>
        <p className="mt-1 text-xs text-steel">Appearance theme (Light, Dark, or System match) and accent swatches saved on this device.</p>
        <div className="mt-3">
          <p className="mb-2 font-mono text-xs uppercase tracking-wider text-steel">Theme Mode</p>
          <ThemeToggle variant="select" />
        </div>
        <div className="mt-4 pt-3 border-t border-hairline-soft">
          <p className="mb-1 font-mono text-xs uppercase tracking-wider text-steel">Accent Color</p>
          <PersonalizeSwatches />
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-hairline bg-canvas p-5 shadow-sm opacity-60">
        ...
        <a href="/files" className="mt-3 inline-flex rounded-full border border-hairline px-4 py-1.5 text-xs font-semibold text-ink hover:border-ink">
          Open Files
        </a>
      </div>
    </div>
...
function AccountsPanel({ user }: { user: WinUser }) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-display text-2xl font-bold text-ink">Accounts</h2>
      ...
      <div className="rounded-xl border border-hairline bg-canvas p-5 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="font-semibold text-ink">{getDisplayName(displayName || user.displayName, user.email)}</p>
        ...
        <div className="mt-4">
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <button onClick={() => fileRef.current?.click()} disabled={uploading} className="rounded-full border border-hairline bg-canvas px-4 py-2 text-xs font-semibold text-ink hover:border-ink disabled:opacity-50">
      ...
      <div className="rounded-xl border border-hairline bg-canvas p-5 shadow-sm">
        <p className="font-semibold text-ink">Your info</p>
        <div className="mt-4 space-y-4">
          <div>
            <label className="font-mono text-xs uppercase tracking-wider text-steel">Display name</label>
            <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Alex Rivera" maxLength={40} className="mt-2 w-full rounded-xl border border-hairline bg-canvas px-4 py-2.5 text-sm text-ink outline-none focus:border-ink" />
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="font-mono text-xs uppercase tracking-wider text-steel">Field / Department</label>
              <select value={department} onChange={(e) => setDepartment(e.target.value)} className="mt-2 w-full rounded-xl border border-hairline bg-canvas px-4 py-2.5 text-sm text-ink outline-none focus:border-ink">
            ...
            <div>
              ...
              <input value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} placeholder="Product Designer" maxLength={40} className="mt-2 w-full rounded-xl border border-hairline bg-canvas px-4 py-2.5 text-sm text-ink outline-none focus:border-ink" />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button onClick={saveProfile} disabled={saving} className="rounded-full bg-ink px-5 py-2 text-xs font-semibold text-canvas hover:opacity-90 disabled:opacity-50">
              {saving ? "Saving…" : "Save"}
            </button>
...
function SecurityPanel({ user }: { user: WinUser }) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-display text-2xl font-bold text-ink">Privacy & security</h2>
      ...
      <div className="rounded-xl border border-hairline bg-canvas p-5 shadow-sm">
        ...
        <form onSubmit={submit} className="mt-4 space-y-3">
          <input type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} placeholder="Current password" className="w-full rounded-xl border border-hairline bg-canvas px-4 py-2.5 text-sm text-ink outline-none focus:border-ink" />
          <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="New password (min 8 chars)" className="w-full rounded-xl border border-hairline bg-canvas px-4 py-2.5 text-sm text-ink outline-none focus:border-ink" />
          <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirm new password" className="w-full rounded-xl border border-hairline bg-canvas px-4 py-2.5 text-sm text-ink outline-none focus:border-ink" />
          {err && <p className="text-xs text-error">{err}</p>}
          {msg && <p className="text-xs text-success-text">{msg}</p>}
          <button type="submit" disabled={saving} className="rounded-full bg-ink px-5 py-2 text-xs font-semibold text-canvas hover:opacity-90 disabled:opacity-50">
      ...
      <div className="rounded-xl border border-hairline bg-canvas p-5 shadow-sm">
        <p className="font-semibold text-ink">Permissions</p>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-5">
        <div className="flex items-center gap-2">
          ...
          <p className="font-semibold text-amber-500">Calls paused</p>
        </div>
      ...
      <div className="rounded-xl border border-hairline bg-canvas p-5 opacity-60 shadow-sm">
        <p className="font-semibold text-ink">Browser permissions</p>
...
function AboutPanel({ user }: { user: WinUser }) {
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-hairline bg-canvas p-5 shadow-sm">
        <p className="font-semibold text-ink">Vaayu Workspace</p>
        ...
        <div className="mt-4 grid gap-3 text-sm">
          ...
          <div className="flex justify-between py-2">
            ...
            <span className="font-mono text-xs text-ink">vaayu-workspace</span>
          </div>
        </div>
      </div>
